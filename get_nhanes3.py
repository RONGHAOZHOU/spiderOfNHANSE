...
def down_nhanes3_file(dataset='nhanes3',types=None):
    ...
    request_url = 'https://wwwn.cdc.gov/nchs/nhanes/nhanes3/DataFiles.aspx'
    ...
    for link in target_links:
        ...
        if not os.path.exists(dir_path):
            os.mkdir(dir_path)
        p.apply_async(down_file,args=[link,dir_path,request_url])
    print('Waiting for all sub processes done....')
    p.close()
    p.join()
    return

def down_file(link,dir_path,request_url):
    # time.sleep(3)
