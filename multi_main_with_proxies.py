...
def get_data(component, year, type='XPT'):
    ...
    r = requests.get(request_url, headers=headers, timeout=600,proxies=proxies)
    ...
    for link in target_links:
        ...
        r = requests.get(link, stream=True,headers=headers,proxies=proxies)
        # download started
...
if __name__ == "__main__":
    ...
    year_list = [2015,2017]  # 2017 ,2019的数据集还未整理，不能下载
    component_list = [
        'Demographics',
                      'Dietary',
                      'Examination',
                      'Laboratory',
                      'Questionnaire'
                      ]  # except Limited Access Data 受限数据需要审批通过下载
    p = Pool()  # 创建进程池
